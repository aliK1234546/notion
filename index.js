...
app.get('/epicPass', async(req, res) => {
 
  var saturdayAndSunday = [true,true] ;
  
  const data = await getData();
  console.log( new Date(data.NoInventoryDays[0]).getDay() )

  data.NoInventoryDays.forEach(day => {
    console.log()
    if(new Date(day).getDay() ===0){
      saturdayAndSunday[1]= false;
...
async function getData(url) {
  ...
  var endDate  =  addDays(today, 15);
  endDate = encodeURIComponent(dateFormat(endDate,"m/d/yyyy"));

  const response = await fetch("https://www.epicpass.com/api/LiftAccessApi/GetCapacityControlReservationInventory?resortCode=44&startDate="+startDate+"&endDate="+endDate+"&_=1610165804515", {
    "headers": {
